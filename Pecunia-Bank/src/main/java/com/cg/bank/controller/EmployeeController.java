...
public class EmployeeController {
	...
	@PostMapping("/login")
	String employeeLogin(@RequestBody EmployeeDto employeeDto){
		employeeService.employeeLogin(employeeDto.geteUsername(), employeeDto.getePassword());
		return util.generateToken(employeeDto);
	}
	
	@PutMapping("/{username}/changepassword")
	ResponseEntity<Void> changePassword(@RequestBody EmployeeDto employeeDto){
	...
	ResponseEntity<Void> forgotPassword(@RequestBody EmployeeDto employeeDto){
		Employee employee = new Employee();
